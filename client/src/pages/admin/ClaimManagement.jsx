/**
 * Claim Processing View (Admin / Staff)
 * Review, investigate, approve with settlement amount, or reject claims
 */

import React, { useState, useEffect } from 'react';
import { FileCheck2, Search, CheckCircle, XCircle, AlertCircle, CheckCircle2, User, FileText } from 'lucide-react';
  ...
  return (
    <div className="app-layout">
      ...
      <div className="main-content">
        <Navbar title="Insurance Claims Processing & Settlement" />
                  ...
                  {filteredClaims.length === 0 ? (
                    <tr>
                      <td colSpan="8" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
                        {loading ? 'Loading claims records...' : 'No claims found.'}
                      </td>
                    </tr>
                  ) : (
                    filteredClaims.map((c) => (
                      <tr key={c.id}>
                        ...
                        <td>
                          ...
                          <span style={{ fontSize: '11.5px', color: 'var(--text-muted)' }}>{c.policy_number}</span>
                        </td>
                        <td>{c.claim_type}</td>
          ...
          <form onSubmit={handleSubmitReview}>
            ...
            <div style={{ padding: '14px', background: 'var(--bg-input)', borderRadius: 'var(--radius-md)', marginBottom: '18px', fontSize: '13px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                <div><strong>Client:</strong> {selectedClaim.client_name} ({selectedClaim.client_phone})</div>
                <div><strong>Policy:</strong> {selectedClaim.policy_name}</div>
                ...
                <div><strong>Claimed Amount:</strong> ₹{Number(selectedClaim.claim_amount).toLocaleString()}</div>
                <div><strong>Policy Coverage:</strong> ₹{Number(selectedClaim.sum_insured || 0).toLocaleString()}</div>
              </div>
              <div style={{ paddingTop: '8px', borderTop: '1px solid var(--border-color)' }}>
                <strong>Description & Incident Details:</strong>
                <p style={{ marginTop: '4px', color: 'var(--text-primary)' }}>{selectedClaim.description}</p>
              </div>
            </div>
            ...
            <div className="form-group">
              <label className="form-label">Staff Remarks & Investigation Notes</label>
              <textarea
                className="form-control"
                rows="3"
                placeholder="Enter verification details, hospital/surveyor report notes, or reason for decision..."
                value={staffRemark}
            ...
            <div className="modal-footer" style={{ padding: '16px 0 0', marginTop: '16px' }}>
              ...
              <button type="submit" className="btn btn-primary">Save Decision & Update Ledger</button>
            </div>
          </form>
        )}
      </Modal>
